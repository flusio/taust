 ...
 * models.
 *
 * It is based on the basic assumption that a model class (under taust\models)
 * using this trait has a corresponding DatabaseModel under taust\models\dao.
...
trait DaoConnector
{
    ...
    {
        if (!self::$dao) {
            $base_class = substr(get_called_class(), strlen('taust\\models\\'));
            $dao_class_name = "\\taust\\models\\dao\\{$base_class}";
